...
generate_imx_sdcard () {
	...
	case "${IMAGE_BOOTLOADER}" in
		...
		dd if=${DEPLOY_DIR_IMAGE}/u-boot_crc.bin.crc of=${SDCARD} conv=notrunc seek=2 bs=512
		dd if=${DEPLOY_DIR_IMAGE}/u-boot_crc.bin of=${SDCARD} conv=notrunc seek=3 bs=512
		;;
		barebox)
...
##eng mode
	# Create partition table
	parted -s ${ENG_SDCARD} mklabel msdos
	parted -s ${ENG_SDCARD} unit KiB mkpart primary fat32 ${IMAGE_ROOTFS_ALIGNMENT} $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED})
	parted -s ${ENG_SDCARD} unit KiB mkpart primary $(expr  ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED}) $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED} \+ $ROOTFS_SIZE)
	parted ${ENG_SDCARD} print
	...
	case "${IMAGE_BOOTLOADER}" in
		...
		;;
		u-boot)
		dd if=${DEPLOY_DIR_IMAGE}/u-boot-${MACHINE}.${UBOOT_SUFFIX_SDCARD} of=${ENG_SDCARD} conv=notrunc seek=2 skip=${UBOOT_PADDING} bs=512
		;;
		barebox)
		dd if=${DEPLOY_DIR_IMAGE}/barebox-${MACHINE}.bin of=${ENG_SDCARD} conv=notrunc seek=1 skip=1 bs=512
		dd if=${DEPLOY_DIR_IMAGE}/bareboxenv-${MACHINE}.bin of=${ENG_SDCARD} conv=notrunc seek=1 bs=512k
		;;
		"")
		;;
		*)
	...
	# Create boot partition image
	BOOT_BLOCKS=$(LC_ALL=C parted -s ${ENG_SDCARD} unit b print \
	                  | awk '/ 1 / { print substr($4, 1, length($4 -1)) / 1024 }')
	mkfs.vfat -n "${BOOTDD_VOLUME_ID}" -S 512 -C ${WORKDIR}/boot.img $BOOT_BLOCKS
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${MACHINE}.bin ::/${KERNEL_IMAGETYPE}
	...
	# Burn Partition
	dd if=${WORKDIR}/boot.img of=${ENG_SDCARD} conv=notrunc seek=1 bs=$(expr ${IMAGE_ROOTFS_ALIGNMENT} \* 1024) && sync && sync
	dd if=${SDCARD_ROOTFS} of=${ENG_SDCARD} conv=notrunc seek=1 bs=$(expr ${BOOT_SPACE_ALIGNED} \* 1024 + ${IMAGE_ROOTFS_ALIGNMENT} \* 1024) && sync && sync
}
